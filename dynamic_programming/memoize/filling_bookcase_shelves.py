...
their thickness is less than or equal to shelfWidth, then build another level of
the shelf of the bookcase so that the total height of the bookcase has increased
...
and second book onto the first shelf, the third book on the second shelf,
...
def min_height_shelves(books: list[list[int]], shelf_width: int) -> int:
    def dp(i):
        ...
        for j in range(i, len(books)):
            ...
            memo[i] = min(memo[i], dp(j + 1) + max_height)
        return memo[i]
...
ic(min_height_shelves([[1, 1], [2, 3], [2, 3], [1, 1], [1, 1], [1, 1], [1, 2]], 4))
