...
def find_rotate_steps(ring: str, key: str) -> int:
    def dp(ring_index, key_index):
        if key_index == len(key):
            return 0

        if (ring_index, key_index) in memoize:
            return memoize[(ring_index, key_index)]
        ...
        for i, c in enumerate(ring):
            if c == key[key_index]:
                minimum_distance = min(
                    abs(i - ring_index),
                    len(ring) - abs(i - ring_index)
                )
                steps = min(
                    steps,
                    minimum_distance + 1 + dp(i, key_index + 1)
                )

        memoize[(ring_index, key_index)] = steps
        return steps
...
ic(find_rotate_steps("godding", "gd"))
