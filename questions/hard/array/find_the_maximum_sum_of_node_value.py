...
where edges[i] = [ui, vi] indicates that there is an edge
between nodes ui and vi in the tree. You are also given a positive integer k,
...
nums[u] = nums[u] XOR k
nums[v] = nums[v] XOR k
Return the maximum possible sum of the values
...
- Choose the edge [0,1].
nums[0] becomes: 2 XOR 7 = 5 and nums[1] become:
3 XOR 7 = 4, and the array nums becomes: [2,3] -> [5,4].
...
Output: 42
Explanation: The maximum achievable sum is 42
which can be achieved by Alice performing no operations.
...
ic(maximum_value_sum([2, 3], 3, [[0, 1], [0, 2]]))
ic(maximum_value_sum([24, 78, 1, 97, 44], 6, [[0, 2], [1, 2], [4, 2], [3, 4]]))
