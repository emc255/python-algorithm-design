...
def minimum_cost_to_hire_workers(quality: list[int], wage: list[int], k: int) -> float:
    result = float('inf')
    pairs = []  # (ratio, quantity)
    ...
    for rate, q in pairs:
        ...
        if len(max_heap) > k:
            total_quantity += heapq.heappop(max_heap)

        if len(max_heap) == k:
            result = min(result, total_quantity * rate)
    return result


ic(minimum_cost_to_hire_workers([3, 1, 10, 10, 1], [4, 8, 2, 2, 7], 3))
