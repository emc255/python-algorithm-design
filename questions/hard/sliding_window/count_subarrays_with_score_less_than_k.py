...
The score of an array is defined as the product of
its sum and its length.
...
because their scores are 10 and 36 respectively, while we need
scores strictly less than 10.
...
def count_subarrays(nums: list[int], k: int) -> int:
    ...
    for right in range(n):
        ...
        result += right - left + 1  # all subarrays ending at right and starting from [left, right]

    return result


ic(count_subarrays(nums=[2, 1, 4, 3, 5], k=10))
