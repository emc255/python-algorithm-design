...
difference between the frequency of two characters, freq[a] - freq[b],
...
The input is generated that at least one substring has a character
...
def max_difference(s: str, k: int) -> int:
    def update_best(best, freq):
        odds = [f for f in freq if f % 2 == 1]
        evens = [f for f in freq if f % 2 == 0 and f > 0]
        if not odds or not evens:
            return best
        current = max(o - e for o in odds for e in evens)
        return max(best, current) if best is not None else current

    n = len(s)
    result = None  # to track if any valid pair is found
    ...
    for window_size in range(k, n + 1):
        ...
        result = update_best(result, freq)

        # Slide window
        for start in range(1, n - window_size + 1):
            freq[int(s[start - 1])] -= 1
            freq[int(s[start + window_size - 1])] += 1
            result = update_best(result, freq)

    return result if result is not None else -1


def max_difference_v2(s: str, k: int) -> int:
    def getStatus(cnt_a2: int, cnt_b2: int) -> int:
        return ((cnt_a2 & 1) << 1) | (cnt_b2 & 1)
...
ic(max_difference_v2(s="12233", k=4))
ic(max_difference_v2(s="330130303114333", k=13))
