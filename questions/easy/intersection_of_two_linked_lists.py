...
the nodes with value 1 in A and B (2nd node in A and 3rd node in B) are different node references.
...
def get_intersection_node(head_a: ListNode, head_b: ListNode) -> Optional[ListNode]:
    ...
    queue_b = deque()
    while head_a:
        queue_a.append(head_a.val)
        head_a = head_a.next
    while head_b:
        queue_b.append(head_b.val)
        head_b = head_b.next
    print(queue_a)
    print(queue_b)
    while queue_b and queue_a:
        a = queue_a.pop()
        b = queue_b.pop()
        if a == b:
            temp = a
