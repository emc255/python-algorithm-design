...
Custom Judge:
The inputs to the judge are given
as follows (your program is not given these inputs):

intersectVal - The value of the node where the intersection occurs.
This is 0 if there is no intersected node.
listA - The first linked list.
listB - The second linked list.
skipA - The number of nodes to skip ahead
in listA (starting from the head) to get to the intersected node.
skipB - The number of nodes to skip ahead
in listB (starting from the head) to get to the intersected node.
...
Explanation:
The intersected node's value is 8 (note that
...
the nodes with value 1 in A and B
...
while the nodes with value 8 in A and B
...
Explanation:
The intersected node's value is 2 (note that this must
...
def get_intersection_node(head_a: ListNode, head_b: ListNode) -> Optional[ListNode]:
    ...
    # Align both pointers for the longer list
...
nodes_A = LinkedList([4, 1, 8, 4, 5]).head
nodes_B = LinkedList([5, 6, 1, 8, 4, 5]).head
get_intersection_node(nodes_A, nodes_B).print()
