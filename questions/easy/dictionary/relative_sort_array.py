...
in arr1 are the same as in arr2. Elements that do not appear
...
def relative_sort_array(arr1: list[int], arr2: list[int]) -> list[int]:
    ...
    for num in arr2:
        v = arr1_counter.pop(num, None)
        if v is not None:
            result.extend([num] * v)
        else:
            result.append(num)

    sorted_arr1_counter = dict(sorted(arr1_counter.items()))
    for k, v in sorted_arr1_counter.items():
        result.extend([k] * v)

    return result


ic(relative_sort_array([2, 3, 1, 3, 2, 4, 6, 7, 9, 2, 19], [2, 1, 4, 3, 9, 6]))
