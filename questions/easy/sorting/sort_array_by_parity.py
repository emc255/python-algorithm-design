"""
905. Sort Array By Parity

Given an integer array nums, move all the even integers at
the beginning of the array followed by all the odd integers.

Return any array that satisfies this condition.

Example 1:
Input: nums = [3,1,2,4]
Output: [2,4,3,1]
Explanation: The outputs [4,2,3,1], [2,4,1,3], and [4,2,1,3] would also be accepted.

Example 2:
Input: nums = [0]
Output: [0]

Constraints:
1 <= nums.length <= 5000
0 <= nums[i] <= 5000

"""
from icecream import ic


def sort_array_by_parity(nums: list) -> list:
    def quicksort(array: list) -> list:
        if len(array) <= 1:
            return array

        middle_value = array[len(array) // 2]

        left_array = [value for value in array if value > middle_value]
        middle_array = [value for value in array if value == middle_value]
        right_array = [value for value in array if value < middle_value]

        return quicksort(left_array) + middle_array + quicksort(right_array)

    nums = quicksort(nums)

    even_numbers = []
    odd_numbers = []
    for n in nums:
        if n % 2 == 0 or n == 1:
            even_numbers.append(n)
        else:
            odd_numbers.append(n)

    return even_numbers + odd_numbers


def sort_array_by_parity_v2(nums: list) -> list:
    i = 0
    j = len(nums) - 1

    while i < j:
        while i < j and nums[i] % 2 == 0:
            i += 1
        while i < j and nums[j] % 2 == 1:
            j -= 1

        nums[i], nums[j] = nums[j], nums[i]

    return nums


ic(sort_array_by_parity([6, 3, 1, 4, 2, 8]))
ic(sort_array_by_parity([1, 1, 2, 2, 3, 1]))
ic(sort_array_by_parity_v2([6, 3, 1, 4, 2, 8]))
ic(sort_array_by_parity_v2([1, 1, 2, 2, 3, 1]))
