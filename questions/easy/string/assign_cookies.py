...
def find_content_children(g: list[int], s: list[int]) -> int:
    ...
    g_index, s_index, g_length, s_length = 0, 0, len(g), len(s)
    while g_index < g_length:
        while s_index < s_length and g[g_index] > s[s_index]:
            s_index += 1
        if s_index == s_length:
            break
...
ic(find_content_children([1, 2, 3], [3]))
ic(find_content_children([1, 2, 3], [1, 1]))
