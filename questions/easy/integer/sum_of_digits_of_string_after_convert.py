...
def get_lucky(s: str, k: int) -> int:
    # Step 1: Convert the string into a numerical string by mapping each letter
    result_str = ""
    for c in s:
        result_str += str(ord(c) - ord("a") + 1)
...
def get_lucky_v2(s: str, k: int) -> int:
    # Step 1: Convert the string into a numerical string by mapping each letter
    result = sum(int(digit) for c in s for digit in str(ord(c) - ord("a") + 1))

    # Step 2: Perform the transformation `k-1` times since the first sum is already done
    for _ in range(k - 1):
