...
def minimum_bit_flips(start: int, goal: int) -> int:
    # Convert the integers to binary strings without the '0b' prefix
    start_string = bin(start)[2:]
    goal_string = bin(goal)[2:]

    # Pad the shorter string with leading zeros
    max_len = max(len(start_string), len(goal_string))
    start_string = start_string.zfill(max_len)
    goal_string = goal_string.zfill(max_len)

    result = 0
    # Loop through the binary strings and compare each bit
    for i in range(max_len):
        if start_string[i] != goal_string[i]:
            result += 1

    return result


def minimum_bit_flips_v2(self, start: int, goal: int) -> int:
    c = 0
    while start or goal:
        if start % 2 != goal % 2:
            c += 1
...
ic(minimum_bit_flips(10, 7))
ic(minimum_bit_flips(3, 4))
ic(minimum_bit_flips(81, 87))
