...
difficulty[i] and profit[i] are the difficulty
and the profit of the ith job, and worker[j] is the
ability of jth worker (i.e., the jth worker can only complete
...
def max_profit_assignment_v2(difficulty: list[int], profit: list[int], worker: list[int]) -> int:
...
def max_profit_assignment(difficulty: list[int], profit: list[int], worker: list[int]) -> int:
    ...
    for w in worker:
        ...
        for i, d in enumerate(difficulty):
            if w >= difficulty[i] and temp_profit < profit[i]:
...
ic(max_profit_assignment([2, 4, 6, 8, 10], [10, 20, 30, 40, 50], [4, 5, 6, 7]))
