...
def merge_nodes_v2(head: Optional[ListNode]) -> Optional[ListNode]:
    temp = head.next
    th = head
    s = 0
    while temp.next:
...
nodes = LinkedList([0, 3, 1, 0, 4, 5, 2, 0])
ic(merge_nodes(nodes.head))
ic(merge_nodes_v2(nodes.head).print())
