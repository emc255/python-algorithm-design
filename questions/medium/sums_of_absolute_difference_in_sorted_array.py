...
def get_sum_absolute_differences_v2(nums: list) -> list:
    n = len(nums)
    total_sum = sum(nums)
    current_left_sum = 0
    result = [0] * n
    ...
    for i, num in enumerate(nums):
        result[i] = i * num - current_left_sum + (total_sum - current_left_sum - (n - i) * num)
...
ic(get_sum_absolute_differences_v2([2, 3, 5]))
