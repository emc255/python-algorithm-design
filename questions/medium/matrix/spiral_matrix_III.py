...
every position in this grid. Whenever you move outside
the grid's boundary, we continue our walk outside
the grid (but may return to the grid boundary later.).
Eventually, we reach all rows * cols spaces of the grid.

Return an array of coordinates representing the positions
of the grid in the order you visited them.
...
def spiral_matrix_III(rows: int, cols: int, row_start: int, col_start: int) -> list[list[int]]:
    result = []
...
# ic(spiral_matrix_III(5, 6, 1, 4))
ic(spiral_matrix_III(1, 4, 1, 3))
