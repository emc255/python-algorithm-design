...
This is because the 2nd row contains security devices,
which breaks the second condition.
...
def number_of_beams(bank: list[str]) -> int:
    ...
    prev_laser_count = 0
    for laser in bank:
        laser_count = laser.count("1")
        if prev_laser_count and laser_count:
            result = result + (prev_laser_count * laser_count)
        if laser_count:
            prev_laser_count = laser_count
    return result


ic(number_of_beams(["011001", "000000", "010100", "001000"]))
ic(number_of_beams(["000", "111", "000"]))
