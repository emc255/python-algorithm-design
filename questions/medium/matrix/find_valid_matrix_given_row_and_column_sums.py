...
the jth column of a 2D matrix. In other words, you do not know the elements of the matrix,
...
def restore_matrix(row_sum: list[int], column_sum: list[int]) -> list[list[int]]:
    R = len(row_sum)
    C = len(column_sum)
    result = [[0] * C for _ in range(R)]

    for r in range(R):
        result[r][0] = row_sum[r]

    for c in range(C):
        current_column_sum = 0
        for r in range(R):
            current_column_sum += result[r][c]

        row_index = 0
        while current_column_sum > column_sum[c]:
            difference = current_column_sum - column_sum[c]
            temp = min(result[row_index][c], difference)
            result[row_index][c] -= temp
            result[row_index][c + 1] += temp
            current_column_sum -= temp
            row_index += 1

    return result


ic(restore_matrix([5, 7, 10], [8, 6, 8]))
