...
def check_subarray_sum(nums: list[int], k: int) -> bool:
    # Dictionary to store the remainder of the cumulative sum and the index
    remainder_dict = {0: -1}
    cumulative_sum = 0
...
def check_subarray_sum_v2(nums: list[int], k: int) -> bool:
    for i in range(len(nums)):
        total = nums[i]
...
ic(check_subarray_sum([23, 2, 6, 4, 7], 6))
ic(check_subarray_sum([23, 2, 6, 4, 7], 13))
