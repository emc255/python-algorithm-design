...
that each element in nums appears in exactly one of the
subsequences.
...
Since three subsequences were created,
we return 3. It can be shown that 3 is the minimum number of subsequences needed.
...
def partition_array(nums: list[int], k: int) -> int:
    ...
    while i < n:
        # Start a new group with nums[i] as the minimum
        group_min = nums[i]
        result += 1
        # Keep adding elements to the current group while they satisfy the condition
...
ic(partition_array(nums=[2, 2, 4, 5], k=0))
ic(partition_array(nums=[1, 2, 3], k=1))
ic(partition_array(nums=[3, 6, 1, 2, 5], k=2))
