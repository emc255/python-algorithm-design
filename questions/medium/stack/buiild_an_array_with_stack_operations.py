...
Use the two stack operations to make the numbers in
the stack (from the bottom to the top) equal to target.
...
If, at any moment,
the elements in the stack (from the bottom to the top) are equal to target,
...
def build_array(target: list, n: int) -> list:
    stack = []
    target.sort()
    i = 0

    for val in range(1, n + 1):
        if i < len(target) and val == target[i]:
            stack.append("Push")
            i += 1
        else:
            stack.append("Push")
