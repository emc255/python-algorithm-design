...
Alice wants the rope to be colorful.
 ...
 ou are given a 0-indexed integer array neededTime
 where neededTime[i] is the time (in seconds) that
...
def min_cost(colors: str, needed_time: list[int]) -> int:
    ...
    while l < n:
        while r < n and colors[l] == colors[r]:
            r += 1
        if len(needed_time[l:r]) > 1:
            arr = sorted(needed_time[l:r])
            result += sum(arr[:-1])
