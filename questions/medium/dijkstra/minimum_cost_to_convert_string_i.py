...
you can pick a character x from the string and change
it to the character y at a cost of z if there exists any index j
such that cost[j] == z, original[j] == x, and changed[j] == y.

Return the minimum cost to convert the string source to
the string target using any number of operations. If it is impossible
to convert source to target, return -1.

Note that there may exist indices i, j such
that original[j] == original[i] and changed[j] == changed[i].
...
Output: 12
Explanation: T
o change the character 'a' to 'b' change the character 'a' to 'c' at a cost of 1,
    ...
    def dijkstra(source_node):
        ...
        while heap:
            ...
            if node in min_cost_map:
                continue
            min_cost_map[node] = node_cost
            for neighbor, neighbor_cost in adjacent[node]:
                heapq.heappush(heap, (node_cost + neighbor_cost, neighbor))
        return min_cost_map

    result = 0
    adjacent = defaultdict(list)

    for src, dst, current_cost in zip(original, changed, cost):
        adjacent[src].append((dst, current_cost))

    minimum_cost_maps = {c: dijkstra(c) for c in set(source)}

    for src, dst in zip(source, target):
        if dst not in minimum_cost_maps[src]:
            return -1
        result += minimum_cost_maps[src][dst]

    return result


ic(minimum_cost("abcd", "acbe", ["a", "b", "c", "c", "e", "d"],
