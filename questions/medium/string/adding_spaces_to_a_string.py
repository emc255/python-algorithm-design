...
in "LeetcodeHelpsMeLearn". We then place spaces before those characters.
...
Explanation:
We are also able to place spaces before the first character of
the string.
...
def add_spaces(s: str, spaces: list[int]) -> str:
    ...
    for i in range(len(s)):
        if index_space < len(spaces) and i == spaces[index_space]:
            result.append(" ")
            result.append(s[i])
            index_space += 1
        else:
...
def add_spaces_v2(s: str, spaces: list[int]) -> str:
    index, result = 0, []
...
ic(add_spaces(s="LeetcodeHelpsMeLearn", spaces=[8, 13, 15]))
