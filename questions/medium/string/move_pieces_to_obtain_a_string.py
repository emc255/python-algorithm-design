...
to the left only if there is a blank space directly to its left,
...
Output: false
Explanation:
The 'R' piece in the string start can move one step to
the right to obtain "_RL_". After that, no pieces can move anymore,
...
Output: false
Explanation:
The piece in the string start can move only to the right,
so it is impossible to obtain the string target from start.
...
def can_change(start: str, target: str) -> bool:
    # Step 1: Remove '_' and check alignment of 'L' and 'R'
    if start.replace('_', '') != target.replace('_', ''):
        return False

    # Step 2: Check positions of 'L' and 'R'
    ...
    for (char, start_idx), (_, target_idx) in zip(start_positions, target_positions):
        if char == 'L' and start_idx < target_idx:
            return False
        if char == 'R' and start_idx > target_idx:
            return False

    return True


ic(can_change(start="_L__R__R_", target="L______RR"))
